...
function ChatTurnBubble({
  ...
  // Only render portals after mount (client-side) to avoid SSR mismatches
  useEffect(() => setMounted(true), [])

  // Lock body scroll while fullscreen is open + ESC to close
  useEffect(() => {
  ...
  function handlePrint() {
    // Open a new window with just the answer content and trigger print
    ...
    printWin.document.write(`
      ...
      <html>
      <head>
        <title>WonderWhiz - ${styleData?.label ?? 'Explanation'}</title>
        <style>
          body { font-family: system-ui, -apple-system, sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; color: #1a1a1a; line-height: 1.7; }
          h1 { font-size: 18px; color: #555; border-bottom: 2px solid #eee; padding-bottom: 8px; }
          h2, h3 { color: #333; }
          ...
          .footer { margin-top: 24px; padding-top: 12px; border-top: 1px solid #eee; font-size: 12px; color: #999; }
        </style>
      </head>
      <body>
        <h1>${subjectData?.emoji ?? ''} ${subjectData?.label ?? ''} · ${styleData?.label ?? 'Explanation'} · Class ${turn.grade}</h1>
        ${answerRef.current?.innerHTML ?? ''}
        <div class="footer">Generated by WonderWhiz AI Tutor</div>
      </body>
      </html>
    `)
  ...
  async function handleSaveImage() {
    // Use the browser's native approach: render the answer to a canvas via an iframe
    // then trigger download. For simplicity, we use a printable screenshot approach.
    try {
      const content = answerRef.current
      if (!content) return

      // Try using the html2canvas approach via dynamic import
      // If not available, fall back to printing
      const html2canvas = (await import('html2canvas' as string).catch(() => null)) as
      ...
      if (html2canvas) {
        ...
        link.download = `wonderwhiz-${turn.subject}-${Date.now()}.png`
        link.href = canvas.toDataURL('image/png')
        link.click()
        toast.success('Image saved!')
      } else {
        // Fallback: use print-to-PDF approach
        handlePrint()
      }
    } catch {
      // Fallback to print
  ...
  // Shared action buttons used in both inline and fullscreen views
  ...
  // Header info shared between inline and fullscreen
  const headerInfo = (
  ...
  // The answer body (shared between inline and fullscreen)
  const answerBody = (
    <div ref={answerRef} className="ww-prose ww-prose-compact">
      ...
      >
        <div className="w-full max-w-[95%] rounded-2xl rounded-bl-md border border-border/60 bg-card p-3 shadow-sm sm:max-w-[90%]">
          ...
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-2">
            {headerInfo}
            {!turn.loading && !turn.error && turn.answer && actionButtons('inline')}
          </div>
